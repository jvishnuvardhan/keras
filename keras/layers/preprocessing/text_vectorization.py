...
class TextVectorization(base_preprocessing_layer.PreprocessingLayer):
  ...
  Args:
    ...
    vocabulary: Optional. Either an array of strings or a string path to a text
      file. If passing an array, can pass a tuple, list, 1-d numpy array, or 1-d
  ...
  def merge_state(self, layers):
    layers = [l._index_lookup_layer for l in layers]  # pylint: disable=protected-access
    self._index_lookup_layer.merge_state(layers)
