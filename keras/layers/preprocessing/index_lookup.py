...
class IndexLookup(base_preprocessing_layer.PreprocessingLayer):
  ...
  Args:
    ...
    vocabulary: Optional. Either an array or a string path to a text file. If
      passing an array, can pass a tuple, list, 1-d numpy array, or 1-d tensor
  ...
  def set_vocabulary(self, vocabulary, idf_weights=None):
    ...
    Args:
      vocabulary: Either an array or a string path to a text file. If passing an
        array, can pass a tuple, list, 1-d numpy array, or 1-d tensor containing
        the vocbulary terms. If passing a file path, the file should contain one
        line per term in the vocabulary.
      idf_weights: A tuple, list, 1-d numpy array, or 1-d tensor of inverse
  ...
  def merge_state(self, layers):
    for l in layers:
      tokens, new_counts = l.token_counts.export()
      old_counts = self.token_counts.lookup(tokens)
      self.token_counts.insert(tokens, new_counts + old_counts)
      if self.output_mode == TF_IDF:
        tokens, new_counts = l.token_document_counts.export()
        old_counts = self.token_document_counts.lookup(tokens)
        self.token_document_counts.insert(tokens, new_counts + old_counts)
        self.num_documents.assign_add(l.num_documents)

  def finalize_state(self):
    if tf.equal(self.token_counts.size(), 0):
  ...
  def call(self, inputs):
    ...
    inputs = self._standardize_inputs(inputs,
                                      tf.int64 if self.invert else self.dtype)
    ...
    return self._encode_output(lookups)
  ...
  def _lookup_dense(self, inputs):
    ...
    # critical for SavedModel, which expects an uninitialized StaticHashTable and
    # will trace the model before restoring the table.
  ...
  def _encode_output(self, lookups):
    ...
    original_shape = lookups.shape
    # In all cases, we should uprank scalar input to a single sample.
    if lookups.shape.rank == 0:
      lookups = expand_dims(lookups, -1)
